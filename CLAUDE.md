...
6. Extinction recovery — if `EXTINCTION_RECOVERY` is on and herbivores/carnivores have hit zero, reseed a small population to keep the simulation going.
7. Effects are aged and expired effects are dropped.
8. Draw phase — entities, then effects on top, then HUD stats, then the population history graph in the top-right.
...
| `EFFECT_KILL` / `EFFECT_EAT_PLANT` / `EFFECT_BIRTH` / `EFFECT_DEATH` | (color, max_radius, duration) tuples for the four flash types |
| `EXTINCTION_RECOVERY`, `EXTINCTION_RESPAWN_*` | Toggle and size of reseed populations |
